...
public class CommentDao extends GenericDaoImpl<Comment, Long> {
	...
	public Comment createComment(String content, Article article, User user) {
		...
		if (Util.IsEmptyOrNull(content))
			throw new ValueNotProvidedException("Content Not provided",
					"content");
	...
	public Comment getCommentOfUserByNamedQuery(String username, long commentId) {
		...
		try {
			...
			Comment comment = (Comment) sessionUtil
					.getSession()
					.getNamedQuery(
			...
			if (comment == null)
				throw new DataNotFoundException("This comment doesnt not exist");
			return comment;
	...
	public List<Comment> getAllCommentsOfUserByNamedQuery(String username) {
		...
		try {
			...
			if (comments.isEmpty())
				throw new DataNotFoundException("No Comments for the "
						+ username);
	...
	public Comment getCommentOfArticleByNamedQuery(long articleId,
			long commentId) {
	...
	public boolean IsArticleCommentAvailable(Long commentId, long articleId) {
		...
		try {
			...
			// throw exception if not present
				...
				throw new DataForbiddenException(
						" comment requested doesnt belong to this article");
	...
	public void decrementCommentCount(Comment comment) {
		...
		try {
			...
			int userCommentCount = user.getCommentCount(); // get count of
															// comments
			int articleCommentCount = article.getCommentCount();
			// throw exception if no articles written
			if (articleCommentCount == 0)
				throw new DataNotFoundException("No comments article");
			if (userCommentCount == 0)
				throw new DataNotFoundException("No comments written by User "
						+ user.getUsername());
